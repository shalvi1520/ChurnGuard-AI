...
//     setRecipientEmail(email.contactEmail || '');
//     setEditMode(false);
//   };
...
//     const to = recipientEmail || selectedEmail.contactEmail;
//     if (!to) {
//       addToast({ type: 'error', message: "Enter the customer's email first" });
//       return;
//     }
//     try {
//       await outreachService.approveEmail(selectedEmail.id);
//       await outreachService.sendEmail(selectedEmail.id, to);
//       const updated = { ...selectedEmail, status: 'sent', contactEmail: to };
//       setSelectedEmail(updated);
//       setEmails(prev => prev.map(e => e.id === selectedEmail.id ? updated : e));
//       addToast({ type: 'success', message: 'Approved and sent to the customer' });
//     } catch (e) {
//       addToast({ type: 'error', message: e?.response?.data?.detail || 'Could not send the email' });
//     }
//   };
...
//   //   if (!selectedEmail) return;
//   //   try {
//   //     await outreachService.sendEmail(selectedEmail.id, recipientEmail || selectedEmail.contactEmail);
//   //     const updated = { ...selectedEmail, status: 'sent', contactEmail: recipientEmail || selectedEmail.contactEmail };
//   //     setSelectedEmail(updated);
//   //     setEmails(prev => prev.map(e => e.id === selectedEmail.id ? updated : e));
//   //     addToast({ type: 'success', message: 'Email sent to the customer' });
//   //   } catch (e) {
//   //     addToast({ type: 'error', message: e?.response?.data?.detail || 'Could not send the email' });
//   //   }
//   // };
...
//                           className="w-full mt-1 px-3 py-2 bg-bg-tertiary/50 border border-border rounded-lg text-sm text-text-primary outline-none focus:border-accent"
//                         />
//                       ) : (
//                         <p className="text-text-primary font-medium break-words">
//                           {selectedEmail.contactEmail || `Customer ${selectedEmail.customerId}`}
//                         </p>
//                       )}
//                     </div>
//                     <div>
...
//                         {selectedEmail.status !== 'sent' && (
//                           <Button size="sm" icon={Send} onClick={handleApproveAndSend}>Approve and Send</Button>
//                         )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-border flex-wrap">
                    {editMode ? (
                      ...
                      <>
                        <Button variant="secondary" size="sm" icon={Edit} onClick={() => setEditMode(true)} disabled={sending}>Edit</Button>
                        {selectedEmail.status !== 'sent' && (
                          <Button size="sm" icon={Send} loading={sending} disabled={sending} onClick={handleApproveAndSend}>
