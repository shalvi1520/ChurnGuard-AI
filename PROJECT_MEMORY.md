...
- **`ui/Disclosure.jsx`** (NEW, session 19) — **the** expand/collapse affordance: a labelled `<button>` with `aria-expanded` over detail worth keeping but not worth making everyone read. Used by Data Management's "View processing details", UploadStep's "What does ChurnGuard look for in my file?" and Portfolio & Risk's "How is this calculated?". Content is mounted only while open. Nothing essential goes inside — the summary above it must stand on its own. **Don't add a fourth inline copy of this pattern.**
...
- **`data-setup/CompleteStep.jsx`** (rewritten session 10, restructured session 19) — not a "you're done" screen but the connected-dataset control centre: provenance, measured facts, which of the user's columns fill which ChurnGuard field, what cleaning ran, and what was left out. Reads only `AppContext.activeDataset`, so opening Data Management with a dataset connected makes **zero API calls**. **Session 19 split it into summary and detail:** a six-stat Dataset Summary and a counted "How ChurnGuard used your data" strip are always visible; the field mapping, extra/excluded columns and cleaning list sit behind one `Disclosure`. Nothing was deleted to make the page shorter — if you add a fact here, decide which of the two layers it belongs in, and don't move detail back up into the summary.
...
**2026-09-24 (session 19)** — UI/UX polish pass: Data Management progressive disclosure, Executive View recomposed, Notifications hidden from the UI.

*Why:* user request — a product-design pass over a working application. No feature was added or removed beyond hiding Notifications; nothing on the backend was touched (zero backend files changed, zero dependency changes).

**Data Management — summary first, detail on request.** `CompleteStep.jsx` showed everything it knew at once: measured stats, the full field mapping, every excluded column, every cleaning step. All of it is still there, in three layers instead of one:
1. **Dataset summary** — six scannable stats (Rows, Columns, Blank values, Duplicate rows, churn examples, Customers scored). "Empty values" is now "Blank values".
2. **How ChurnGuard used your data** — a short plain-language line plus five counted facts (required/optional fields matched, extra columns used, columns left out, issues fixed), each with an InfoTip.
3. **"View processing details"** — one disclosure holding the full column mapping, the extra columns used, the excluded columns *grouped by reason* (six near-identical lines became three), and the automatic cleaning.
Required/optional field counts moved out of the stats grid into layer 2, where they belong (they describe mapping, not the file). `CompleteStep.test.jsx` pins the split: summary always visible, detail never rendered until expanded, nothing lost when it is.

**Executive View recomposed.** The page had a large unexplained empty region. Fixed by rearranging the existing cards, not by adding content:
- The centred CHURNGUARD logo block (a third copy of the brand, after the sidebar and header) became a compact banner: shield + page title + one-line purpose, with PREDICT → EXPLAIN → ACT on the right. The footer line that repeated the same tagline is gone.
- Charts are a 5/7 split at `xl` instead of 50/50 — the drivers chart carries long category labels, the donut does not. The donut's `ResponsiveContainer width="50%"` (which fought its flex parent) is now a capped, centred chart + legend pair, with each tier's share of the total beside its count.
- Critical accounts and the retention estimate share the bottom row (7/5, `items-start`) instead of being two full-width strips. "Critical Accounts Requiring Immediate Action" is now "Accounts to act on first"; its table headers carry glossary InfoTips.
- Chart fills come from `getRiskColor()` + `resolvedTheme` instead of hardcoded dark-theme hex, so the light theme is right.

**Notifications hidden from the UI (frontend only).** Removed the bell from the desktop header and the mobile header, the mounted `NotificationPanel`, and the Settings → Notifications tab with its preference toggles (nothing read them; there is no notification delivery). **`features/notifications/NotificationPanel.jsx`, `mock/notifications.js`, `notificationService` and the `notifications`/`unreadCount` state in `AppContext` are all untouched** — this was a visibility decision, not a teardown. Nothing dispatches `SET_NOTIFICATIONS` now, so the state simply stays empty and costs nothing.

**`ui/Disclosure.jsx` (NEW)** — the expand/collapse affordance existed as two inline copies (UploadStep's "What does ChurnGuard look for in my file?", Portfolio & Risk's "How is this calculated?"). Both now use the shared component, as does Data Management's new one. Real `<button>`, `aria-expanded`, content mounted only while open. **Don't write a third inline copy.**

**Real bug found and fixed while auditing charts:** Explainability's factor chart had `domain={[-0.15, 0.4]}` hardcoded, so any factor stronger than 0.4 — exactly the ones worth seeing — was drawn clipped at the axis edge. It now fits the account's own factors with zero kept on the axis, the same way Portfolio & Risk does it. Its hardcoded `#F97316`/`#4ADE80`/`#2A2F42` fills were replaced with theme-resolved ones at the same time.

**Wording.** `shapContribution` is "Factors Affecting Churn Risk" (was "Feature Contributions"), described in plain language with SHAP named in the `help` rather than the title. `impactScore`'s help no longer leads with "SHAP effect". Data Management's page description is one line instead of three.

**Recall removed from History's dataset cards** (user request, 2026-09-25) — the stat row is Rows / Columns / First uploaded, three columns instead of four. A model-quality metric is not what someone chooses between saved datasets on. The backend still returns `metrics.recall`; nothing on the frontend reads it now.

**Smaller polish:** Settings capped to `max-w-3xl` (four short forms under a full-width tab rule read as unfinished) and its Organization inputs made uncontrolled — `value` with no `onChange` warned on every mount; Customers' filter row given one label style and equal-width selects; Recommendations' cards use their full width instead of shrink-wrapping; Outreach's audit-trail card has an empty state instead of rendering an empty card, and its copy matches its own button ("Approve and Send"); driver-name truncation matched between Portfolio & Risk and Executive View.

**Verified:** `npm run build` clean, `npm run lint` exit 0 (pre-existing warnings only), `npm test` 119/119 (8 new). Visually checked in Chromium via Playwright against a stubbed API — Data Management (collapsed and expanded), Executive View, Portfolio & Risk, Customers, Customer Detail, Explainability, Recommendations, Outreach, History and Settings, in both themes, at 1920/1440/1024/390 — no console errors, no horizontal overflow, no bell icon anywhere.

*Files:* new `src/components/ui/Disclosure.jsx`, `src/components/data-setup/CompleteStep.test.jsx`, `src/pages/ExecutiveOverviewPage.test.jsx`. Changed: `CompleteStep.jsx`, `UploadStep.jsx`, `AppLayout.jsx`, `DashboardPage.jsx`, `DataManagementPage.jsx`, `ExecutiveOverviewPage.jsx`, `ExplainabilityPage.jsx`, `CustomersPage.jsx`, `RecommendationsPage.jsx`, `OutreachPage.jsx`, `SettingsPage.jsx`, `utils/glossary.js`. **No backend file was modified.**
...
- **Don't re-add a Data tab to Settings.** Data belongs to Data Management and History. Settings keeps Profile / Organization / Security / Appearance. **Notifications left that list in session 19** — the tab and the header bell are hidden at the user's request; re-adding either means shipping actual notification delivery first, not just the toggles back.
